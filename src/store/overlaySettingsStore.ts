  ...
  (set, get) => ({
      ...
      try {
        ...
        if (!idToUse) {
          ...
          const { data, error } = await supabase
            .from("overlay_settings")
            .insert([
              { ...get().defaultSettings, ...settings, user_id: userId },
            ])
            .select();
    ...
    deleteAudio: async (type: "increment" | "decrement" | "reset") => {
      try {
        ...
        if (soundUrl) {
          const filePath = `${userId}/${type}_sound.${soundUrl
            .split(".")
            .pop()}`;
          const { error } = await supabase.storage
            .from("overlay_assets")
    ...
    updateSettings: async (newSettings: Partial<OverlaySettings>) => {
